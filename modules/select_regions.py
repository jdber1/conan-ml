...
def set_drop_region(experimental_drop, experimental_setup):
    ...
    if experimental_setup.drop_ID_method == "Automated":
        ...
        experimental_setup.drop_region = user_ROI(
            experimental_drop.image, 'Select drop region', scale, screen_position)
        experimental_drop.cropped_image = image_crop(
            experimental_drop.image, experimental_setup.drop_region)
...
def user_ROI(raw_image, title,  scale, screen_position):
    global drawing
    global ix, iy
    global fx, fy
    global image_TEMP
    ...
    while (1):
        ...
        if DRAW_TANGENT_LINE_WHILE_SETTING_BASELINE:
            if experimental_setup.tangent_boole == True or experimental_setup.second_deg_polynomial_boole == True or experimental_setup.circle_boole == True or experimental_setup.ellipse_boole == True:
                ...
                perform_fits(experimental_drop, tangent=experimental_setup.tangent_boole, polynomial=experimental_setup.second_deg_polynomial_boole,
                             circle=experimental_setup.circle_boole, ellipse=experimental_setup.ellipse_boole)
        ...
        if k != 255:
            ...
            if (k == 0):  # up key (down on image)
                ...
                if TEMP:
                    ...
                    image_TEMP = raw_image.copy()
                img = image_TEMP.copy()
                # line_colour,line_thickness)    cv2.line
                cv2.line(img, (ix, iy), (fx, fy), (0, 255, 0), 2)
