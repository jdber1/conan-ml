...
#         plt.show()
        ...
        if i == 0:
            ...
            filename = user_inputs.filename[:-4] + \
                '_' + user_inputs.time_string + ".csv"
            export_filename = os.path.join(
                user_inputs.directory_string, filename)
        ...
        if user_inputs.baseline_method == "Automated":
            if user_inputs.tangent_boole == True or user_inputs.second_deg_polynomial_boole == True or user_inputs.circle_boole == True or user_inputs.ellipse_boole == True:
                perform_fits(raw_experiment, tangent=user_inputs.tangent_boole, polynomial=user_inputs.second_deg_polynomial_boole,
                             circle=user_inputs.circle_boole, ellipse=user_inputs.ellipse_boole)
        ...
        if user_inputs.ML_boole == True or user_inputs.YL_boole == True:
            ...
            if user_inputs.ML_boole == True:
                ...
                raw_experiment.contact_angles['ML model']['angles'] = [
                    ML_predictions[0, 0], ML_predictions[1, 0]]
...
if __name__ == '__main__':
    ...
    buttonFont = tkFont.Font(family='Helvetica', size=48, weight='bold')
